    ...
    public class Runtimes : STARUIBase<Runtime, DownloadedQuest, InstalledRuntime>
    {
        public Runtimes(Guid avatarId) : base(new API.ONODE.Core.Managers.RuntimeManager(avatarId),
            "Welcome to the Runtime Wizard", new List<string> 
            {
                ...
                "Finally you run the sub-command 'runtime publish' to convert the folder containing the runtime (can contain any number of files and sub-folders) into a OASIS Runtime file (.oruntime)..",
                "You can then share the .oruntime file with others from which you can create OAPP's from (along with a OAPP Template, you can even use the same OAPP Template for different runtimes).",
